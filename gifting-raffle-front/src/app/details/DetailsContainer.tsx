...
export const DetailsContainer: React.FC = () => {
  ...
  const { error, payload, loading } = useQuery<RaffleDetails>(getRaffleDetailsAction(raffleId));

  const { mutate, loading: mutationLoading } = useMutation(closeRaffleAction);

  return <Details close={mutate} loading={loading || mutationLoading} raffle={payload} error={error} />;
};
